/**
 *  This example demonstrate how we can use lars::parser to define a command-line calculator and 
 *  use a visitor pattern to evaluate the result.
 */
...
int main() {
  ...
  using Expression = lars::Interpreter<void, Visitor&>::Expression;

  struct Visitor {
    float result;
    unordered_map<string, float> variables;
    
    float getValue(Expression e){ e.evaluate(*this); return result; }
    ...
    void visitAssignment(Expression name, Expression value){ result = (variables[name.string()] = getValue(value)); }
    void visitNumber(Expression value){ result = stod(value.string()); }
  };
  
  lars::ParserGenerator<void, Visitor&> calculator;
  ...
  g["Multiply"  ] << "Product '*' Exponent"  >> [](auto e, auto &v){ v.visitMultiplication(e[0],e[1]); };
